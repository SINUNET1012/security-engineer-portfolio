...
def build_code_cell(it: dict) -> str:
    ...
    lines.append("from checks.unix import run as run_check")
    lines.append("")
...
def find_notebook_path(code: str) -> Path:
    matches = list(UNIX_ROOT.rglob(f"{code}_*.ipynb"))
    if len(matches) != 1:
...
def main() -> int:
    ...
    u_items = [it for it in items if str(it.get("code", "")).startswith("U-")]
    ...
    print(f"updated {updated} UNIX notebooks")
    return 0
