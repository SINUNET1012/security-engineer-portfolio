...
def build_code_cell(it: dict) -> str:
    ...
    lines.append("from checks.web import run as run_check")
    lines.append("")
...
def find_notebook_path(code: str) -> Path:
    matches = list(WEB_ROOT.rglob(f"{code}_*.ipynb"))
    if len(matches) != 1:
...
def main() -> int:
    ...
    web_items = [it for it in items if str(it.get("code", "")).startswith("WEB-")]
    ...
    print(f"updated {updated} WEB notebooks")
    return 0
