...
def build_code_cell(it: dict) -> str:
    ...
    lines.append("from checks.windows import run as run_check")
    lines.append("")
...
def find_notebook_path(code: str) -> Path:
    matches = list(WIN_ROOT.rglob(f"{code}_*.ipynb"))
    if len(matches) != 1:
...
def main() -> int:
    ...
    w_items = [it for it in items if str(it.get("code", "")).startswith("W-")]
    ...
    print(f"updated {updated} Windows notebooks")
    return 0
