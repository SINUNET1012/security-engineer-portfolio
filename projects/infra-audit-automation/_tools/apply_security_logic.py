...
def build_code_cell(it: dict) -> str:
    ...
    lines.append("from checks.security import run as run_check")
    lines.append("")
...
def find_notebook_path(code: str) -> Path:
    matches = list(SEC_ROOT.rglob(f"{code}_*.ipynb"))
    if len(matches) != 1:
...
def main() -> int:
    ...
    s_items = [it for it in items if str(it.get("code", "")).startswith("S-")]
    ...
    print(f"updated {updated} Security(S) notebooks")
    return 0
